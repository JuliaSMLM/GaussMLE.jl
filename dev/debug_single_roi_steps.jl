...
for n_iter in iteration_counts
    ...
    println("\n[$n_iter iterations]")
    println("  x: $(round(e.x, digits=3)) (Δ = $(round(e.x - x_true, digits=3)), σ=$(round(e.σ_x, digits=3)))")
    println("  y: $(round(e.y, digits=3)) (Δ = $(round(e.y - y_true, digits=3)), σ=$(round(e.σ_y, digits=3)))")
    println("  z: $(round(e.z, digits=3)) (Δ = $(round(e.z - z_true, digits=3)), σ=$(round(e.σ_z, digits=3)))")
    ...
    push!(results_at_iters, (n_iter=n_iter, emitter=e))
end

# Generate model PSFs at each iteration
model_psfs = []
for (n_iter, e) in results_at_iters
    model_roi = zeros(Float32, box_size, box_size)
    fitted_params = Float32[e.x, e.y, e.z, e.photons, e.bg]
...
for (col, (title, img)) in enumerate(zip(titles, [roi_data[:,:,1], model_psfs...]))
    ...
    # Mark fitted center if not ground truth
    if col > 1
        e = results_at_iters[col-1].emitter
        scatter!(ax, [e.x], [e.y], color=:yellow, markersize=12, marker=:xcross)
    end
end
...
Label(fig[3, :],
      ...
      join([let e = r.emitter
             "$(r.n_iter) iter | " *
             "$(round(e.x,digits=2)) ($(round(e.x-x_true,digits=2))) | " *
             "$(round(e.y,digits=2)) ($(round(e.y-y_true,digits=2))) | " *
             "$(round(e.z,digits=1)) ($(round(e.z-z_true,digits=1))) | " *
...
e0 = results_at_iters[1].emitter
if abs(e0.x - x_true) < 1.0 && abs(e0.y - y_true) < 1.0
    println("  ✓ Initialization: x,y within 1 pixel of truth")
else
    println("  ✗ Initialization: x,y OFF by $(round(e0.x-x_true,digits=2)), $(round(e0.y-y_true,digits=2)) pixels")
end

# Check first iteration
e1 = results_at_iters[2].emitter
if abs(e1.x - x_true) < abs(e0.x - x_true)
    println("  ✓ First iteration: x improved")
else
    println("  ✗ First iteration: x got WORSE (Δ: $(round(e0.x-x_true,digits=3)) → $(round(e1.x-x_true,digits=3)))")
end

if abs(e1.y - y_true) < abs(e0.y - y_true)
    println("  ✓ First iteration: y improved")
else
    println("  ✗ First iteration: y got WORSE (Δ: $(round(e0.y-y_true,digits=3)) → $(round(e1.y-y_true,digits=3)))")
end

# Check convergence
e10 = results_at_iters[3].emitter
if abs(e10.x - x_true) < 0.1 && abs(e10.y - y_true) < 0.1
    println("  ✓ After 10 iter: x,y converged to truth")
else
    println("  ✗ After 10 iter: x,y still OFF by $(round(e10.x-x_true,digits=2)), $(round(e10.y-y_true,digits=2))")
end
