...
println("Comprehensive Benchmark: 10^4 Samples per Model")
println("=" ^ 80)

# Test parameters
n_rois = 10_000  # Reduced for reasonable runtime
boxsize = 7
n_photons = 1000
bg_photons = 10
σ_psf = 1.5
...
function generate_test_data_fixed(model_type::Symbol, n_rois::Int, boxsize::Int; 
    ...
    if model_type == :xynb
        true_params[:x] = fill(x_true, n_rois)
        true_params[:y] = fill(y_true, n_rois)
        true_params[:n] = fill(n_true, n_rois)
        true_params[:bg] = fill(bg_true, n_rois)
    ...
    elseif model_type == :xynbs
        ...
        true_params[:n] = fill(n_true, n_rois)
        true_params[:bg] = fill(bg_true, n_rois)
        true_params[:σ] = fill(σ_true, n_rois)
    ...
    elseif model_type == :xynbsxsy
        σx_true = Float32(1.4)  # Slightly asymmetric
        σy_true = Float32(1.6)
        true_params[:x] = fill(x_true, n_rois)
        true_params[:y] = fill(y_true, n_rois)
        true_params[:n] = fill(n_true, n_rois)
        true_params[:bg] = fill(bg_true, n_rois)
        true_params[:σx] = fill(σx_true, n_rois)
        true_params[:σy] = fill(σy_true, n_rois)
        
        θ = GaussMLE.θ_xynbsxsy(x_true, y_true, n_true, bg_true, σx_true, σy_true)
        args = GaussMLE.GaussModel.Args_xynbsxsy{Float32}()
        
    elseif model_type == :xynbz
        z_true = Float32(0.2)  # 200nm above focal plane
        true_params[:x] = fill(x_true, n_rois)
        true_params[:y] = fill(y_true, n_rois)
        true_params[:z] = fill(z_true, n_rois)
        true_params[:n] = fill(n_true, n_rois)
        true_params[:bg] = fill(bg_true, n_rois)
...
function compute_statistics(fitted_params, true_params, param_names, uncertainties=nothing)
    ...
    for (i, param) in enumerate(param_names)
        ...
        true_val = true_params[param][1]  # All true values are the same
...
# Test each model
models = [:xynb, :xynbs, :xynbsxsy, :xynbz]
model_names = Dict(
    :xynb => "Basic 4-parameter (x,y,n,bg)",
    :xynbs => "5-parameter with PSF width (x,y,n,bg,σ)",
    :xynbsxsy => "6-parameter asymmetric PSF (x,y,n,bg,σx,σy)",
...
for model in models
    ...
    elseif model == :xynbsxsy
        cpu_fitted = [[θ.x, θ.y, θ.n, θ.bg, θ.σ_x, θ.σ_y] for θ in θ_cpu]
        cpu_uncert = [[σ.σ_x, σ.σ_y, σ.σ_n, σ.σ_bg, σ.σ_σx, σ.σ_σy] for σ in Σ_cpu]
    elseif model == :xynbz
    ...
    if CUDA.functional()
        ...
        elseif model == :xynbsxsy
            gpu_fitted = [[θ.x, θ.y, θ.n, θ.bg, θ.σ_x, θ.σ_y] for θ in θ_gpu]
            gpu_uncert = [[σ.σ_x, σ.σ_y, σ.σ_n, σ.σ_bg, σ.σ_σx, σ.σ_σy] for σ in Σ_gpu]
        elseif model == :xynbz
...
if CUDA.functional()
    ...
    for model in models
        ...
        if haskey(results, model_str)
            ...
            println("True values: ", results[model_str]["true_params"])
            ...
            # Header
            println(@sprintf("\n%-10s %20s %20s", "", "CPU", "GPU"))
            println(@sprintf("%-10s %20s %20s", "Parameter", "Bias | Std | CRLB", "Bias | Std | CRLB"))
            println("-"^52)
